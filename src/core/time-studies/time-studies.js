...
function showSecondPreferredWarning(currTree) {
  ...
  if (canPickSecond && TimeStudy.preferredPaths.dimension.path.length < 2) {
    GameUI.notify.error("Вы не выбрали второй приоритет пути в развилке измерений.");
    return true;
  }
  return false;
}
...
export function buyStudiesUntil(id, ec = -1) {
  ...
  if (ec === 11 && ecHasRequirement) {
    ...
    GameUI.notify.error("Вы не выбрали предпочитительный путь в развилке измерений.");
    return studyArray;
  }
  ...
  if (V.isFullyCompleted && !Pelle.isDoomed) {
    ...
    GameUI.notify.error("Вы не выбрали предпочитительный путь в развилке темпа.");
    return studyArray;
  }
