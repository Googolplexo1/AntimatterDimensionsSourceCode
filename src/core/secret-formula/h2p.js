...
<br>
<br>
Вы также можете в любой момент сбросить своё сохранение по кнопке "СБРОСИТЬ ЭТО СОХРАНЕНИЕ". Вам будет предложено ввести
...
столько времени, сколько вы отсутствовали в игре. Например, при ${formatInt(1000)} офлайн-тиков, если вы отсутствовали
...
Галактики Репликанти - это новый вид галактик, которые дают тот же самый эффект, что и Галактики Антиматерии, и
...
уровня. Его условие заключается в том, что <i>порядки</i> множителей <i>всех</i> измерений и скорости тика возведены в степень
${format(0.75, 2, 2)}. (Это первый появившийся из эффектов такого типа в игре, которые мы за неимением общеупотребительного
...
определённого типа в степень, присутствует на всех глифах соответствующего типа.
<br>
...
<br>
После ${format(1e30)} МР наценка возрастает на ${format(0.2, 1, 1)} каждую покупку, после ${format(Number.MAX_VALUE, 1)}
...
её эффект аннулируется и экспоненциально восстанавливается в течение ${BlackHoles.ACCELERATION_TIME} секунд по реальному времяисчислению.
...
${Ra.unlocks.enslavedUnlock.canBeApplied
    ...
    : "<span style='color: var(--color-bad);'>Разблокируйте Память Безымянных, чтобы просмотреть информацию.</span>"}
<br>
<br>
<b>Ви</b>
<br>
...
${Ra.unlocks.vUnlock.canBeApplied
    ? `Каждые ${formatInt(6)} уровней разблокируется новое <i>Тройственное Исследование</i>. Это особые Исследования ` +
      ...
      `На ${formatInt(6)}-м уровне разблокируются три <i>супердостижения Ви</i>, каждое из которых имеет по пять уровней. ` +
      "Как и обычные достижения Ви, они могут быть выполнены лишь внутри Реальности Ви. Каждый выполненный уровень " +
      "супердостижения Ви приравнивается к двум выполненным уровням обычных достижений Ви и, в частности, даёт две " +
      "Теоремы Пространства, а не одну."
    : "<span style='color: var(--color-bad);'>Разблокируйте Память Ви, чтобы просмотреть информацию.</span>"}
`,
      isUnlocked: () => VUnlocks.raUnlock.isUnlocked,
      tags: ["заряженное", "зарядить", "улучшение", "бесконечности", "расширение", "глифов", "пульсирование",
        "тройственное", "исследование", "супердостижение", "ви", "ра"],
      tab: ""
    }, {
      name: "Алхимия",
      info: () => `
...
значения для его уровня на его редкость. Кроме того, существует глобальное ограничение на все алхимические ресурсы, равное ${format(25000)}.
<br>
<br>
...
производится определённой алхимической реакцией, которую вы можете включить или выключить по желанию. Алхимические реакции
...
вы могли бы получить на реальности, если бы ограничения не существовало. Мнимые Машины пассивно производятся так, что их
количество приближается к ограничению экспоненциально по реальному времяисчислению (каждую минуту реального времени разность
...
уже разблокировали - это эффект Тёмной Материи. При отключённом континууме эти множители не имеют эффекта, так что вы теряете
часть производства.
`,
      isUnlocked: () => Laitela.isUnlocked,
...
<br>
<br>
Ещё одна отличительная особенность Измерений Тёмной Материи заключается в том, что их производство не является непрерывным.
Вместо этого каждое из них осуществляет единичный объём производства с некоторым интервалом по реальному времяисчислению,
...
<br>
<br>
У каждого Этапа Сингулярности есть значок, указывающий, какого рода эффект он даёт:
<br>
