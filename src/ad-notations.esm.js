...
var VOLUME_UNITS = [[0, "пл", 0, ""], [12299410000000 * 0.005, "шкалик", 1, "m"], [12299410000000 * 0.01, "чарка", 2, "f"], [12299410000000 * 0.02, "четушка", 3, "f"], [12299410000000 * 1 / 48, "стакан", 4, "m"], [12299410000000 * 0.025, "косушка", 5, "f"], [12299410000000 * 0.05, "полуштоф", 6, "m"], [12299410000000 * 0.0625, "винн бутылка", 7, "f"], [12299410000000 * 0.075, "бутылка", 8, "f"], [12299410000000 * 0.1, "штоф", 9, "m"], [12299410000000 * 0.125, "осьмериков штоф", 9, "m"], [12299410000000 * 0.25, "гарнец", 9, "m"], [12299410000000 * 1, "ведро", 7, ""], [12299410000000 * 2, "корчага", 5, "f"], [12299410000000 * 40, "бочка", 2, "f"]];
var MINIMS = VOLUME_UNITS[1];
var VOLUME_ADJECTIVES = ["ничтожн", "мельчайш", "крошечн", "мал", "средн", "крупн", "умеренн", "больш", "обширн", "велик", "огромн", "исполинск", "необъятн", "миров", "вселенск", "бесконечн", "вечн", "реальн", "квантов", "трансфинитн", "абсолютн", "парадоксальн", "воображаем", "обречённ", "предельн"];
...
var ImperialNotation = function (_super) {
  ...
  ImperialNotation.prototype.convertToVolume = function (x, adjective) {
    ...
    if (x <= MINIMS[0] * 1.5) {
      return this.decline(MINIMS, 1, adjective);
    }
    ...
    if (remainder < VOLUME_UNITS[volIdx - big[2]][0]) {
      return this.decline(big, numBig, adjective);
    }
  ...
  ImperialNotation.prototype.formatMetric = function (x) {
    if (x < 1000) {
      return "".concat(x < 10 || x === Math.round(x) ? x.toFixed(2) : x.toFixed(0), "пл");
    }

    if (x < 1e6) {
      return "".concat((x / 1000).toPrecision(4), "нл");
    }

    if (x < 1e9) {
      return "".concat((x / 1e6).toPrecision(4), "мкл");
    }

    return "".concat((x / 1e9).toPrecision(4), "мл");
  };
  ...
  ImperialNotation.prototype.checkAlmost = function (adjective, x, numBig, bigIndex) {
    ...
    if (x + Math.max(VOLUME_UNITS[bigIndex - big[2]][0], MINIMS[0] * 0.5) > big[0]) {
    ...
    if (x + small[0] >= big[0]) {
      return this.shortOf(adjective, numBig + 1, big, small);
    }
  ...
  ImperialNotation.prototype.bigAndSmall = function (adjective, numBig, big, numSmall, small) {
    var bigStr = this.decline(big, numBig, adjective);
    return numSmall === 0 ? bigStr : "".concat(bigStr, " и ").concat(this.decline(small, numSmall));
  };

  ImperialNotation.prototype.almost = function (adjective, numBig, big) {
    return "почти ".concat(this.decline(big, numBig, adjective));
  };

  ImperialNotation.prototype.shortOf = function (adjective, numBig, big, small) {
    return "".concat(this.decline(big, numBig, adjective), " без ").concat(this.decline(small, 0));
  };

  ImperialNotation.prototype.decline = function (noun, num, adjective) {
    var grammar = "many";
    if (num <= 4) grammar = "few";
    if (num === 1) grammar = "singular";

    var adjStr = adjective === undefined ? "" : this.declineAdjective(adjective, grammar, noun[3]);
    var numStr = num <= 1 ? "" : num.toString().concat(" ");
    return "".concat(numStr, adjStr, this.declineNoun(noun[1], grammar, noun[3]))
  };

  ImperialNotation.prototype.declineAdjective = function (word, grammar, gender) {
    var firstLetter = ["мельчайш", "средн", "больш", "велик", "исполинск", "вселенск"].includes(word) ? "и" : "ы";
    var ending = firstLetter.concat("х");
    if (grammar === "singular") {
      ending = ["мельчайш", "средн"].includes(word) ? "ее" : "ое"
      if (gender === "m") ending = ["больш", "миров"].includes(word) ? "ой" : firstLetter.concat("й");
      if (gender === "f") ending = word === "средн" ? "яя" : "ая";
    }
    return word.concat(ending, " ");
  };

  ImperialNotation.prototype.declineNoun = function (word, grammar, gender) {
    var words = word.split(" ");
    if (words.length === 2) return this.declineAdjective(words[0], grammar, gender).concat(this.declineNoun(words[1], grammar, gender));
    switch (grammar) {
      case "singular":
        return word;
      case "few":
        if (word === "гарнец") return "гарнца";
        if (gender === "m") return word.concat("а");
        return word.slice(0, -1).concat("и");
      case "many":
        if (word === "гарнец") return "гарнцев";
        if (word === "корчага") return "корчаг";
        if (gender === "m") return word.concat("ов");
        if ("чш".includes(word.at(-3))) return word.slice(0, -2).concat("ек");
        return word.slice(0, -2).concat("ок");
    }
  };
...
export { AllNotation, BarNotation, BlindNotation, BlobsNotation, BracketsNotation, ClockNotation, CustomNotation, DotsNotation, EmojiNotation, EngineeringNotation, HexNotation, ImperialNotation, InfinityNotation, LettersNotation, LogarithmNotation, MixedEngineeringNotation, MixedScientificNotation, Notation, PrimeNotation, RomanNotation, ScientificNotation, Settings, ShiNotation, StandardNotation, ZalgoNotation, abbreviateStandard, formatMantissa, formatMantissaBaseTen, formatMantissaWithExponent };
