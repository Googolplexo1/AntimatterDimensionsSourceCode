...
export default {
  ...
  computed: {
    baseSpeedText() {
      if (this.isStopped) {
        return `${formatInt(0)} (запущено сохранение реального времени)`
      }
      const speed = this.formatNumber(this.baseSpeed);
      if (this.isEC12) {
        return `${speed} (фиксированно)`;
      }
      return `${speed}`;
    },
    pulseSpeedText() {
    ...
    baseText() {
      ...
      return this.baseSpeed === 1
        ? "Игра идёт с нормальной скоростью."
        : `Скорость игры изменена: ${this.baseSpeedText}`;
    }
  },
  methods: {
    update() {
